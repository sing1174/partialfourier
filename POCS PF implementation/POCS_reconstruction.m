% K space reconstruction 6/8 5/8 4/8
clear;clc;
loadedData = load('sample_data.mat'); 
variableNames = fieldnames(loadedData);
firstVariable = loadedData.(variableNames{1});

newVariable= permute(firstVariable, [3 2 1 4]);


% Change channel and slice here
kspace = squeeze(newVariable(14, :, :, 5));
original_image = abs(ifftshift(ifft2(kspace)));


a =zeros(148,148);
kspace68 = complex(a,a);
kspace68(38:148,:) = kspace;

image_68 = abs(ifftshift(ifft2(kspace68)));
...
[imReco_58, kspPF_58] = pocs(kspace58, iter, watchProgress);
